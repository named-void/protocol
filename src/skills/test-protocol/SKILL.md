...
Удаления по role/status/датам, захардкоженному id или capture из GET-lookup запрещены; restore-`PUT`/`PATCH` cleanup этого ограничения не имеет. Если ячейке нужна существующая запись под fixture, помечай её lookup-`GET` в `prepare` флагом `"prerequisite": true`.
Цель назначения системного поля (например `is_contact = true`) бери с уже установленным значением: cleanup раннера восстанавливает только захваченный объект, а перенос признака с прежнего держателя невосстановим.
...
Auth-диагностика: при `FAIL` ячейки, противоречащем постановке задачи, разрешён read-only `SELECT` permissions её ролей из БД auth на том же postgres-сервере; подтверждённое принципиальное разрешение или запрет контура для сбойной пары укажи в результате как наиболее вероятную причину.
