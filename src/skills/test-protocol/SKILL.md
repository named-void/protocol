...
Для `PUT`/`PATCH` добавь `prepare` с `GET` и `save_as`, `json_from` с минимальным `json_patch` и cleanup с восстановлением; для `DELETE` — `prepare` с `POST` уникального fixture, capture его id и cleanup. Cleanup-`DELETE` обязан ссылаться на идентификатор записи, созданной POST-prepare этого сценария (в пути или query — `{имя.пути}` либо `$capture`); удаления по role/status/датам, захардкоженному id или capture из GET-lookup запрещены, restore-`PUT`/`PATCH` cleanup этого ограничения не имеет. Если ячейке нужна существующая запись под fixture, помечай её lookup-`GET` в `prepare` флагом `"prerequisite": true`.
